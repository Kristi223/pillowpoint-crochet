...
import { TRANSLATIONS, LESSONS_I18N } from '../lib/translations';
...
export default function Home() {
  ...
  const LESSONS = LESSONS_I18N[lang];
  ...
  const HOOKS = {
    circle:    { thin: 'hook #4-6 (4.0-5.0mm / US G-H / UK 8-6)', standard: 'hook #6-8 (5.0-6.0mm / US H-J / UK 6-4)', thick: 'hook #8-10 (6.0-10.0mm / US J-N / UK 4-0)' },
    square:    { thin: 'hook #5-7 (4.5-6.0mm / US 7-J / UK 7-4)', standard: 'hook #7-9 (6.0-9.0mm / US J-M / UK 4-0)', thick: 'hook #9-10 (9.0-10.0mm / US M-N / UK -)' },
    triangle:  { thin: 'hook #5-6 (4.5-5.0mm / US 7-H / UK 7-6)', standard: 'hook #6-8 (5.0-6.0mm / US H-J / UK 6-4)', thick: 'hook #8-10 (6.0-10.0mm / US J-N / UK 4-0)' },
    oval:      { thin: 'hook #4-5 (4.0-4.5mm / US G-7 / UK 8-7)', standard: 'hook #6-7 (5.0-6.0mm / US H-J / UK 6-4)', thick: 'hook #8-9 (6.0-9.0mm / US J-M / UK 4-0)' },
    rectangle: { thin: 'hook #4-5 (4.0-4.5mm / US G-7 / UK 8-7)', standard: 'hook #6-7 (5.0-6.0mm / US H-J / UK 6-4)', thick: 'hook #8-9 (6.0-9.0mm / US J-M / UK 4-0)' },
  };
          ...
          <aside style={{ width: 230, flexShrink: 0, background: `linear-gradient(180deg,#fff,${C.blush})`, borderRight: `1px solid ${C.line}`, display: 'flex', flexDirection: 'column', padding: '28px 0 20px', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
              {Object.entries(TRANSLATIONS).map(([key, val]) => (
                <button key={key} onClick={() => setLang(key)} style={{ padding: '4px 8px', borderRadius: 8, border: `1.5px solid ${lang===key ? C.gold : C.line}`, background: lang===key ? C.gold : 'transparent', color: lang===key ? '#fff' : C.brown2, fontFamily: 'Montserrat', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>{val.flag} {val.lang}</button>
              ))}
            </div>
            </div>
            <nav style={{ padding: '20px 12px', flex: 1 }}>
              {[['lessons', '🎬', t.nav.lessons], ['generator', '🧮', t.nav.generator]].map(([s, icon, label]) => (
        ...
        {isMobile && (
          <header style={{ background: '#fff', borderBottom: `1px solid ${C.line}`, padding: '14px 16px', position: 'sticky', top: 0, zIndex: 100 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
              {[['lessons', '🎬 ' + t.nav.lessons], ['generator', '🧮 ' + t.nav.generator]].map(([s, label]) => (
        ...
        <main style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {section === 'lessons' && (
            <div style={{ padding: isMobile ? '24px 16px 60px' : '40px 48px 80px', maxWidth: 900 }}>
              <h1 className="reveal" style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 32, fontWeight: 600, marginBottom: 6 }}>{t.lessons_title}</h1>
          ...
          {section === 'generator' && (
            <div style={{ padding: isMobile ? '24px 16px 60px' : '40px 48px 80px', maxWidth: 860 }}>
              <h1 className="reveal" style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 32, fontWeight: 600, marginBottom: 6 }}>{t.generator_title}</h1>
              ...
              <div style={{ background: '#fff', borderRadius: 20, padding: isMobile ? '24px 18px' : '36px 40px', boxShadow: '0 16px 48px -20px rgba(107,76,59,.18)', border: `1px solid ${C.line}` }}>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    {[['cm', t.unit_cm], ['inch', t.unit_inch]].map(([u, label]) => (
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {[['circle','⭕',t.shapes.circle],['oval','🥚',t.shapes.oval],['square','⬛',t.shapes.square],['rectangle','▬',t.shapes.rectangle],['triangle','🔺',t.shapes.triangle]].map(([s,icon,label]) => (
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
                    {[['thin',t.yarns.thin,'~1 cm / 0.4"'],['standard',t.yarns.standard,'~2 cm / 0.8"'],['thick',t.yarns.thick,'~3 cm / 1.2"']].map(([key,name,sub]) => (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ borderBottom: `2px solid ${C.line}` }}>
                          {[t.table.round, t.table.stitches, t.table.size, t.table.instructions].map(h => (
